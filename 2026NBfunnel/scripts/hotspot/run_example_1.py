...
if use_multiprocessing:
    ...
    max_cpus = 8 # We might want to not run on the full number of cores, as Rosetta take about 2 Gb of memory per instance
...
rosetta_scripts_path = os.path.expanduser("/Users/liute/work/rosetta/rosetta.binary.m1.release-408/main/source/bin/relax.macosclangrelease")
nstruct = 1 # Normally 35
max_minimization_iter = 1000 # Normally 5000
abs_score_convergence_thresh = 1.0 # Normally 1.0
number_backrub_trials = 1000 # Normally 35000
backrub_trajectory_stride = 1000 # Can be whatever you want, if you would like to see results from earlier time points in the backrub trajectory. 7000 is a reasonable number, to give you three checkpoints for a 35000 step run, but you could also set it to 35000 for quickest run time (as the final minimization and packing steps will only need to be run one time).
